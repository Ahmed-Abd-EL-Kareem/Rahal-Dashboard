...
export class DestinationsService {
  ...
  getDestinations(page?: number, limit?: number, search?: string, isActive?: string, category?: string): Observable<any> {
    ...
    return this.http.get<any>(`${environment.apiUrl}/destinations`, { params }).pipe(
      catchError((err) => {
  ...
  getAllDestinations(page?: number, limit?: number, search?: string, isActive?: string, category?: string): Observable<any> {
    return this.getDestinations(page, limit, search, isActive, category);
  }
  ...
  getDestinationBySlug(slug: string): Observable<any> {
    return this.http.get<any>(`${environment.apiUrl}/destinations/slug/${slug}`).pipe(
      catchError((err) => {
        const msg = err.error?.message ?? 'Failed to fetch destination details.';
        return throwError(() => new Error(msg));
      })
