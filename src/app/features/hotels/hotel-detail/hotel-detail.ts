...
//   const h = this.hotel();
//   if (!h) return [];
//   const cover = h.coverImage ? [h.coverImage] : [];
//   const imgs = (h.images ?? []).filter(
//     (img: string) => img && img !== h.coverImage
//   );
//   return [...cover, ...imgs];
// });
...
//         (b: any) => (b.hotel?._id ?? b.hotel)?.toString() === this.hotelId,
//       );
//       console.log('Bookings:', filtered); // ← ضيف ده
//       console.log('Booking dates:', filtered.map((b: any) => ({
//   date: b.createdAt,
//   status: b.status,
//   price: b.totalPrice
// })));
//       console.log('Payment statuses:', filtered.map((b: any) => b.paymentStatus)); // ← وده
//       this.bookings.set(filtered);
...
//     d.setDate(d.getDate() - (6 - i));
//     const label = d.toLocaleDateString('en', { weekday: 'short' });
//     const value = this.bookings() // ← بدلنا filteredBookings بـ bookings عشان نشوف الكل
//       .filter((b) => {
//         const bd = new Date(b.createdAt);
//         return (
//           (b.status === 'confirmed' || b.status === 'completed') &&
...
interface BasicModel {
  nameEn: string; nameAr: string; type: string;
  descriptionEn: string; descriptionAr: string;
}
interface LocationModel {
  city: string; region: string; addressEn: string;
  addressAr: string; lng: string; lat: string;
}
interface RoomModel {
  type: string; pricePerNight: string; capacity: string;
}
...
export class HotelDetailComponent implements OnInit {
  ...
  basicModel = signal<BasicModel>({
    nameEn: '', nameAr: '', type: '', descriptionEn: '', descriptionAr: '',
  });

  locationModel = signal<LocationModel>({
    city: '', region: '', addressEn: '', addressAr: '', lng: '', lat: '',
  });
  ...
  allImages = computed(() => {
    ...
    if (!h) return [];
    const cover = h.coverImage ? [h.coverImage] : [];
    const imgs = (h.images ?? []).filter((img: string) => img && img !== h.coverImage);
    return [...cover, ...imgs];
  });
  ...
  constructor() {
    this.hotelsService.getMeta().pipe(takeUntilDestroyed()).subscribe((meta) => {
      this.CITIES = meta.cities || [];
      this.AMENITIES_LIST = meta.amenities || [];
      this.REGIONS = meta.regions || [];
    });
...
ngOnInit(): void {
  ...
  setTimeout(() => {
    console.log('Chart data:', this.dailyRevenueChart());
    console.log('Max value:', this.maxChartValue());
  }, 3000); // ← بعد 3 ثواني عشان الـ data تيجي
  ...
  loadHotel(): void {
    ...
    this.hotelsService.getHotelById(this.hotelId).subscribe({
      next: (res) => {
        ...
        if (h._id) {
          this.hotelId = h._id;
          this.loadStats(); // ← بعد ما عرفنا الـ ID الحقيقي
        }
  ...
  private patchForms(h: any): void {
    ...
    this.locationModel.set({
      ...
      addressEn: h.address?.en ?? '', addressAr: h.address?.ar ?? '',
      lng: h.location?.coordinates?.[0]?.toString() ?? '',
      lat: h.location?.coordinates?.[1]?.toString() ?? '',
    });
    this.selectedStars = h.stars ?? 4;
    ...
    this.selectedImages = [...(h.images ?? [])];
    this.currency.set(h.currency ?? 'EGP');
    const roomsData: any[] = h.rooms?.length ? h.rooms : [
      { type: 'single', pricePerNight: 800, capacity: 1 },
      { type: 'double', pricePerNight: 1400, capacity: 2 },
      { type: 'suite', pricePerNight: 3500, capacity: 2 },
      { type: 'family', pricePerNight: 1100, capacity: 4 },
    ];
    this.roomModels.set(roomsData.map((r) => signal<RoomModel>({
      type: r.type,
      pricePerNight: r.pricePerNight?.toString() ?? '0',
      capacity: r.capacity?.toString() ?? '1',
    })));
...
loadStats(): void {
  ...
  this.hotelsService.getAllBookings({ limit: 200 }).subscribe({
    next: (res) => {
      ...
      console.log('Booking dates:', filtered.map((b: any) => b.createdAt));
      console.log('Today:', new Date().toDateString());
      console.log('Statuses:', filtered.map((b: any) => b.status));
      this.bookings.set(filtered);
      this.loadingStats.set(false);
    },
    error: () => this.loadingStats.set(false),
...
setStatsRange(r: '7' | '30' | '90'): void {
  ...
  totalBookings = computed(() => this.filteredBookings().length);
  confirmedCount = computed(() =>
    this.filteredBookings().filter((b) => b.status === 'confirmed' || b.status === 'completed').length,
  );
  canceledCount = computed(() =>
    this.filteredBookings().filter((b) => b.status === 'canceled').length,
  );
  ...
  return Array.from({ length: 7 }, (_, i) => {
    ...
    const value = this.bookings()
      .filter((b) => {
        ...
        if (match) console.log('MATCH:', b.createdAt, b.totalPrice); // ← ده
        return match;
      })
      .reduce((s, b) => s + b.totalPrice, 0);
    return { label, value };
...
barPx(value: number): number {
  ...
  if (!max) return 2;
  return Math.max((value / max) * 90, value > 0 ? 4 : 2);
}
  // ── Room Helpers ─────────────────────────────────────────────────────────
  getRoomValue(idx: number): RoomModel { return this.roomModels()[idx](); }
  updateRoom(idx: number, field: keyof RoomModel, value: string): void {
    const s = this.roomModels()[idx];
    s.set({ ...s(), [field]: value });
  }
  ...
  // ── Amenities / Stars ────────────────────────────────────────────────────
  toggleAmenity(a: string): void {
    this.selectedAmenities.has(a) ? this.selectedAmenities.delete(a) : this.selectedAmenities.add(a);
  }
  ...
  private uploadFiles(files: File[]): void {
    ...
    forkJoin(uploads).subscribe({
      next: (results) => {
        results.forEach((r) => { if (r?.secure_url) this.selectedImages.push(r.secure_url); });
        this.uploadingImages.set(false);
      },
      error: () => this.uploadingImages.set(false),
    });
  ...
  saveChanges(): void {
    ...
    this.saving.set(true);
    const rooms = this.roomModels().map((s) => ({
      type: s().type, pricePerNight: +s().pricePerNight, capacity: +s().capacity,
    }));
    const avgPrice = rooms.length ? rooms.reduce((sum, r) => sum + r.pricePerNight, 0) / rooms.length : 0;
    const dto: any = {
      name: { en: bv.nameEn, ar: bv.nameAr || bv.nameEn },
      description: { en: bv.descriptionEn, ar: bv.descriptionAr || bv.descriptionEn },
      type: bv.type, city: lv.city, region: lv.region,
      address: { en: lv.addressEn, ar: lv.addressAr },
      stars: this.selectedStars,
      amenities: Array.from(this.selectedAmenities),
      rooms, averagePricePerNight: Math.round(avgPrice),
      currency: this.currency(),
      images: this.selectedImages,
      coverImage: this.selectedImages[0] ?? null,
      ...(lv.lng && lv.lat ? { location: { type: 'Point', coordinates: [+lv.lng, +lv.lat] } } : {}),
